...
<body>	
	<div class="modal-body" id="Edit">
		<?php
			$candidate_course	= $db->fetch_all_data("candidate_course",[],"id = '".$_GET["data_id"]."'")[0];
			$candidate_id		= $_GET["candidate_id"];
			$candidate_name 	= $db->fetch_single_data("candidates","name",array("id"=>$candidate_id));

			if(isset($_POST["saving_new"])){
				$db->addtable("candidate_course");	$db->where("id",$_GET["data_id"]);
							...
							<?=$f->start("","POST","","enctype='multipart/form-data'");?>
								<div class="row wls-contact-mid">
									...
									<div class="col-md-6 col-sm-6 form-group contact-forms">
										...
										<?=$f->select("degree_id",$db->fetch_select_data("degrees","id","name",[],[],"",true),@$candidate_course["degree_id"],"","select_form");?>
									</div>
								</div>
								<div class="row wls-contact-mid">
									<div class="col-md-6 col-sm-6 form-group contact-forms">
										<font style="color:#1a75ff;font-style:italic;">Course Name</font>
										<?=$f->input("course_name",@$candidate_course["course_name"],"required","form-control");?>
									</div>
									<div class="col-md-6 col-sm-6 form-group contact-forms">
										<font style="color:#1a75ff;font-style:italic;">Course Year</font>
										<?=$f->input("course_year",@$candidate_course["course_year"],"type='number'","form-control");?>
									</div>
								</div>
