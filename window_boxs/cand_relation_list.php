...
<body>			
	<?php
		...
		$candidate_relations = $db->fetch_data(true);
		
		if($_GET["deleting"]){
			$db->addtable("candidate_relations");	$db->where("id",$_GET["deleting"]);
			$deleting = $db->delete_();
	...
	<div class="modal-body" id="List">
		<div class="login mx-auto mw-100">
			<h5 class="text-center">Candidate Relations</h5>
			<?php include_once "../a_notification.php"; ?>
			<div class="table_list">
				<div style="overflow-x:auto;">
					<?=$t->start("","","data_content");?>
						<?=$t->header(["No","Action","Relation","Full Name","Division","Location"]);?>
						<?php 
							$i=0;
							foreach($candidate_relations as $no => $data){
								$i++;
